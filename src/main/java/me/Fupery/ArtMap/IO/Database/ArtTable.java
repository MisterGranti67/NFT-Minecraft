...
final class ArtTable extends SQLiteTable {
    ...
    public MapArt getArtwork2(int id) throws SQLException {
    ...
    void updateUUID(int ID, UUID player) throws SQLException {
        new QueuedStatement() {
        ...
        }.execute("UPDATE " + TABLE + " SET artist=? WHERE id=?;");
    }
    ...
    String[] listID(UUID player) throws SQLException {
        return new QueuedQuery<String[]>() {
            ...
            protected String[] read(ResultSet results) throws SQLException {
                ...
                while (results.next()) {
                    artists.add(results.getString("id"));
                }
                return artists.toArray(new String[artists.size()]);
            }
        }.execute("SELECT DISTINCT id FROM " + TABLE + " WHERE artist = ? ORDER BY id;");
    }

    String[] listTitle(UUID player) throws SQLException {
        return new QueuedQuery<String[]>() {
            ...
            protected String[] read(ResultSet results) throws SQLException {
                ArrayList<String> artists = new ArrayList<>();
                while (results.next()) {
                    artists.add(results.getString("title"));
                }
                return artists.toArray(new String[artists.size()]);
            }
        }.execute("SELECT DISTINCT title FROM " + TABLE + " WHERE artist = ? ORDER BY title;");
    }
    ...
    /**
     * @param number demander l'id du dessin
     * @return
     * @throws SQLException
     */
    String[] trouverTitle(Integer number) throws SQLException {
        return new QueuedQuery<String[]>() {
        ...
        }.execute("SELECT DISTINCT title FROM " + TABLE + " WHERE id = ? ORDER BY id;");
    }

    String[] trouverDate(Integer number) throws SQLException {
        return new QueuedQuery<String[]>() {
            ...
            protected String[] read(ResultSet results) throws SQLException {
                ArrayList<String> artists = new ArrayList<>();
                while (results.next()) {
                    artists.add(results.getString("date"));
                }
                return artists.toArray(new String[artists.size()]);
            }
        }.execute("SELECT DISTINCT date FROM " + TABLE + " WHERE id = ? ORDER BY id;");
    }

    String[] trouverArtist(Integer number) throws SQLException {
        return new QueuedQuery<String[]>() {
            ...
            protected String[] read(ResultSet results) throws SQLException {
                ArrayList<String> artists = new ArrayList<>();
                while (results.next()) {
                    artists.add(results.getString("artist"));
                }
                return artists.toArray(new String[artists.size()]);
            }
        }.execute("SELECT DISTINCT artist FROM " + TABLE + " WHERE id = ? ORDER BY id;");
    }
    ...
    String[] trouvermapid(String title) throws SQLException {
        return new QueuedQuery<String[]>() {
        ...
        }.execute("SELECT DISTINCT id FROM " + TABLE + " WHERE title = ? ORDER BY title;");
    }

    String[] trouverId(String title) throws SQLException {
        return new QueuedQuery<String[]>() {
    ...
    String[] trouverDatetitle(String title) throws SQLException {
        return new QueuedQuery<String[]>() {
        ...
        }.execute("SELECT DISTINCT date FROM " + TABLE + " WHERE title = ? ORDER BY title;");
    }

    String[] trouverArtisttitle(String title) throws SQLException {
        return new QueuedQuery<String[]>() {
        ...
        }.execute("SELECT DISTINCT artist FROM " + TABLE + " WHERE title = ? ORDER BY title;");
    }
